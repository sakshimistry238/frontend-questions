...
export const TestComponent = () => {
  ...
  clientIo.current.on("connect", () => {
    console.log("✅ Connected to server:", clientIo.current.id);
  });

  clientIo.current.on("connect_error", (err:any) => {
...
// console.log(clientIo.current.connect(),"clientIo.current");
        ...
        <div className="w-full flex justify-between mt-10">
          {question && (
            <button
              onClick={() => quitGame()}
              type="button"
              disabled={valid?valid?.winAmount>1000:true}
